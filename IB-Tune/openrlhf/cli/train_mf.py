...
from openrlhf.trainer import MFTrainer
from openrlhf.utils import blending_datasets, get_strategy, get_tokenizer
from openrlhf.utils import DatasetLoader

def sequential_collate_fn(batch):
    # 获取所有不同的 file_name
    file_names = set(item["file_name"] for item in batch)

    if len(file_names) != 1:
        return []
    
    sorted_batch = sorted(batch, key=lambda x: x["idx"])
    processed_batch = []
    for i in range(len(sorted_batch) - 1):
        current = sorted_batch[i]
...
def train(args):
    ...
    loader = DatasetLoader(train_dir=args.dataset , seed=args.seed)
    train_data, eval_data = loader.load_dataset(train_n=args.train_data_n, test_n=2)
    ...
    train_dataloader = strategy.setup_dataloader(
        ...
        shuffle=False,
        drop_last=True,
        collate_fn=sequential_collate_fn if args.sequential_sampling else (
    ...
    scheduler = get_scheduler(
        ...
        scheduler_specific_kwargs={"min_lr": args.learning_rate * 0.01},
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mf_model", type=str)
    ...
    parser.add_argument("--have_mf_model", action="store_true", default=False)
    ...
    # SFT
    parser.add_argument("--train_data_n", type=int, default=200)
    parser.add_argument("--use_true_action", action="store_true", default=True)
    parser.add_argument("--max_epochs", type=int, default=2)
    parser.add_argument("--aux_loss_coef", type=float, default=0, help="MoE balancing loss")
    parser.add_argument("--pretrain", type=str, default=None)
    parser.add_argument("--learning_rate", type=float, default=1e-6)
    parser.add_argument("--mf_coef_max", type=float, default=0.2)  # not use
    parser.add_argument("--lr_warmup_ratio", type=float, default=0.2)
    ...
    parser.add_argument("--sequential_sampling", type=str, default=True)
