...
class SFTDataset(Dataset):
    ...
    def __init__(
        ...
        num_processors=8,
        multiple_of=1,
        sequential_sampling=False,  # New flag
    ) -> None:
        super().__init__()
        ...
        # Extract attributes for normal operation
        self.prompts = self.data["prompt"]
        self.responses = self.data["response"]
        self.prompt_ids_lens = self.data["prompt_ids_len"]
    ...
    def process_data(self, data):
        ...
        if not self.pretrain_mode:
            if isinstance(prompt, dict) and not isinstance(prompt, str):
                prompt_str = (
                        f"{prompt['topic']}"
                        f"{prompt['state']}"
                    )
            ...
            # Filter out samples exceeding max length
            if not prompt or not response or prompt_ids_len >= self.max_length - 2:
                prompt = None
        else:
            prompt_ids_len = 0
    
        # Preserve "file_name" and "idx" in the output
        return {
            "prompt": prompt,
            "response": response,
    ...
    def __getitem__(self, idx):
        if self.sequential_sampling == False:
            ...
            return {
                ...
                "response": self.responses[idx],
                "file_name": self.data["file_name"][idx],  # Ensure file_name exists
                "idx": self.data["idx"][idx],  # Ensure idx exists
            }
